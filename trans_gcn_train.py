...
from lib.utils import collate_wrapper_dynamic
from trans_utils import read_cfg_file, get_log_dir, get_logger, get_device, load_graph_data_adj_mx, get_multi_graph
...
def run_model(model, dataset_type, data_iterator, device, output_dim, scaler, graph_list, sim_rate):
    model.eval()
    y_pred_list = []

    for i, (x, y, xtime, ytime) in enumerate(data_iterator):
        y = y[..., :output_dim]

        edge_index_list, edge_attr_list = get_multi_graph(graph_list, xtime, sim_rate, dataset_type)

        sequences, y = collate_wrapper_dynamic(x=x, y=y,
...
def main(args):
    ...
    # np.savetxt("./result1.txt", adj_mx_list[1], fmt='%.2f')
    ...
    for epoch in range(cfg['train']['epochs']):
        ...
        for _, (x, y, xtime, ytime) in enumerate(train_iterator):
            ...
            edge_index_list, edge_attr_list = get_multi_graph(adj_mx_list, xtime, sim_rate, 'train')
            sequences, y = collate_wrapper_dynamic(x=x, y=y,
                                                   edge_index=edge_index_list,
                                                   edge_attr=edge_attr_list,
                                                   device=device,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_filename',
                        default='data/model/trans_hz_multi_graph.yaml',
                        type=str,
